---
title: "Vehicle Speed Limiting"
description: "Vehicle Speed Limiting (VehSpdLmt) — The Vehicle Speed Limiting Function determines a limited assist torque command value as a function of vehicle"
---

# Vehicle Speed Limiting

*Repository directory: `VehSpdLmt`*

:::tip[Origin: Custom — Nexteer logic on Vector-generated scaffold]
Application logic is Nexteer in-house; the RTE interaction scaffold was generated by the Vector MICROSAR RTE Generator / DaVinci tooling (generator banner in file header, `autosar/` model, `utp/contract` RTE stubs).
:::

## Purpose and responsibility

The Vehicle Speed Limiting Function determines a limited assist torque command value as a function of vehicle speed and handwheel position to manage mechanical fatigue near end-of-travel positions.

## AUTOSAR software component(s)

Modelled SW-C(s) found in `autosar/` (DaVinci/MICROSAR model, AUTOSAR 3.1.4):

- `Ap_VehSpdLmt`

## Key files

Implementation (`src/` or module root):

- `VehSpdLmt/src/Ap_VehSpdLmt.c`

<details>
<summary>Unit-test RTE stubs (`utp/contract/`, Vector-generated, 9 files)</summary>

- `VehSpdLmt/utp/contract/Ap_VehSpdLmt_Cfg.h`
- `VehSpdLmt/utp/contract/CalConstants.h`
- `VehSpdLmt/utp/contract/Compiler_Cfg.h`
- `VehSpdLmt/utp/contract/MemMap.h`
- `VehSpdLmt/utp/contract/Rte.h`
- `VehSpdLmt/utp/contract/Rte_Ap_VehSpdLmt.h`
- `VehSpdLmt/utp/contract/Rte_Compiler_Cfg.h`
- `VehSpdLmt/utp/contract/Rte_MemMap.h`
- `VehSpdLmt/utp/contract/Rte_Type.h`

</details>

RTE/BSW generation templates (`generate/`, consumed by the Vector generators — the outputs land in the ECU project `GenData*` folders, not here):

- `VehSpdLmt/generate/Ap_VehSpdLmt_Cfg.arxml.tt`
- `VehSpdLmt/generate/Ap_VehSpdLmt_Cfg.h.tt`

## Public API (extracted from sources)

Non-static function definitions found in the module `.c` files (file shown for reference; signatures live in the headers above):

| Function | Defined in |
| --- | --- |
| `VehSpdLmt_Per1` | `Ap_VehSpdLmt.c` |

## Typical usage

Call pattern derived from the detected entry points (exact runnable mapping is defined by the RTE configuration in the ECU project):

```c
VehSpdLmt_Per1(/* ports */);  /* periodically, via RTE runnable */
```

## Dependencies

RTE (generated per ECU project, Vector MICROSAR/DaVinci — consumed, not owned):

- `Rte_Ap_VehSpdLmt.h`

Shared project services used:

- `CalConstants.h` (calibration constants / system time / global macros)
- `GlobalMacro.h` (calibration constants / system time / global macros)
- `Std_Types.h` (calibration constants / system time / global macros)

<details>
<summary>All quoted project includes (raw, for traceability)</summary>

- `Ap_VehSpdLmt_Cfg.h`
- `CalConstants.h`
- `GlobalMacro.h`
- `MemMap.h`
- `Std_Types.h`
- `fixmath.h`
- `interpolation.h`

</details>

## Documents

The module ships 1 Word/PDF/text documentation file(s). Summaries and conversion notes live on the documents page:

- [Design and reference documents](./documents/)

Source files:

- `VehSpdLmt/doc/VehSpdLmt_MDD.docx`
